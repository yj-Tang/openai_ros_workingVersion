...
def StartOpenAI_ROS_Environment(task_and_robot_environment_name):
    """
    It doesthe robot all the stuff that the user would have to do to make it simpler
    for the user
    This means:
    1) Registers the Task Environemnt wanted, if it exists in the Task_Envs. ########## why register the Task Env? ##########
    2) Checks that the workspace of the user has all that is needed for launching this.
    which means that it will check that the robot spawn launch is there and the worlds spawn is there.
    3) Launches the world launch and robot spawn.    ####################### what i am concerned ############################
    4) It will import the gym Env and Make it.
    """
    rospy.logwarn("Env: {} will be imported".format(
        task_and_robot_environment_name))
    result = RegisterOpenAI_Ros_Env(task_env=task_and_robot_environment_name, 
    max_episode_steps=10000)             # register and import

    if result:
        rospy.logwarn("Register of Task Env went OK, lets make the env..."+str(task_and_robot_environment_name))
        env = gym.make(task_and_robot_environment_name)   # gym make
    else:
...
class ROSLauncher(object):
    def __init__(self, rospackage_name, launch_file_name, ros_ws_abspath="/home/user/simulation_ws"):
        ...
        try:
            pkg_path = self.rospack.getpath(rospackage_name)
        ...
        if pkg_path:
            ...
            assert False, "NO Package Pth was found for package " + str(rospackage_name)
